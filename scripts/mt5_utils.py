...
def collect_maximum_data(symbol: str, timeframe) -> Optional[pd.DataFrame]:
    ...
    print(f"  Initial range: {earliest_datetime} to {datetime.fromtimestamp(df_temp['time'].max(), tz=pytz.utc)}")
    ...
    for attempt in range(max_attempts):
        ...
        if earlier_rates is None or len(earlier_rates) == 0:
            print(f"  [OK] No more historical data available (reached limit at attempt {attempt + 1})")
            break
        ...
        if (attempt + 1) % 2 == 0:  # Print every 2 attempts
            print(f"    Attempt {attempt + 1}: {len(all_rates):,} bars total, earliest: {earliest_datetime}")
...
def save_dataframe(df: pd.DataFrame, filepath: Path, save_format: str = "parquet"):
    ...
    if save_format in ["parquet", "both"]:
        ...
        print(f"  [OK] Saved parquet: {parquet_path.name} ({size_mb:.2f} MB)")
    
    if save_format in ["csv", "both"]:
        csv_path = filepath.with_suffix('.csv')
        df.to_csv(csv_path, index=False)
        size_mb = os.path.getsize(csv_path) / (1024 * 1024)
        print(f"  [OK] Saved CSV: {csv_path.name} ({size_mb:.2f} MB)")
...
def append_new_data(existing_df: pd.DataFrame, new_df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"  [OK] Merged data: {len(existing_df):,} existing + {len(new_df):,} new = {len(combined):,} total")
    if duplicates_removed > 0:
